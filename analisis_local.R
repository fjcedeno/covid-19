...
df<-SUPERFICIE_COMUNAL[POBLACION_COMUNAL,on="CODIGO_COMUNA"]
df<-MARCO_COMUNAL[df,on="CODIGO_COMUNA"]
df<-df[PASO_COMUNAL,on="CODIGO_COMUNA"]
DF_UCI_REGIONAL<-data.table::copy(UCI_REGIONAL[,-c("REGION_RESIDENCIA"),with=FALSE])
DF_UCI_REGIONAL[,CODIGO_REGION:=as.numeric(CODIGO_REGION)]
colregional<-c('CAMAS_UCI_HABILITADAS','CAMAS_UCI_OCUPADAS_COVID_19','CAMAS_UCI_OCUPADAS_NO_COVID_19','CAMAS_BASE_2019')
data.table::setnames(DF_UCI_REGIONAL,old=colregional,new=paste0(colregional,"_REGIONAL"))
df<-DF_UCI_REGIONAL[df,on=c("SEMANA","CODIGO_REGION"),allow.cartesian=TRUE]


DF_UCI_NACIONAL<-data.table::copy(UCI_NACIONAL[,-c("REGION_RESIDENCIA"),with=FALSE])
colnacional<-c('CAMAS_UCI_HABILITADAS','CAMAS_UCI_OCUPADAS_COVID_19','CAMAS_UCI_OCUPADAS_NO_COVID_19','CAMAS_BASE_2019')
data.table::setnames(DF_UCI_NACIONAL,old=colnacional,new=paste0(colnacional,"_NACIONAL"))
df<-DF_UCI_NACIONAL[df,on=c("SEMANA","FECHA"),allow.cartesian=TRUE]

df<-COVID_CASOS_ACTIVOS[df,on=c("SEMANA","CODIGO_COMUNA")]

DF_COVID_CASOS_POSITIVIDAD<-data.table::copy(COVID_CASOS_POSITIVIDAD[,-c("REGION_RESIDENCIA"),with=FALSE])
df<-DF_COVID_CASOS_POSITIVIDAD[df,on=c("FECHA","SEMANA","CODIGO_REGION")]

DF_COVID_CASOS_CONFIRMADOS_COMUNAS<-data.table::copy(COVID_CASOS_CONFIRMADOS_COMUNAS[,-c("POBLACION"),with=FALSE])

df<-DF_COVID_CASOS_CONFIRMADOS_COMUNAS[df,on=c("FECHA","SEMANA","CODIGO_COMUNA")]

df<-na.omit(df)

df<-df[CODIGO_COMUNA=="13101"]

semanas<-sort(unique(df$SEMANA))
N<-4
NT<-20
data<-list()
for(i in 1:(length(semanas)-NT-N))
{
  data[[i]]<-list(train=df[SEMANA%in%semanas[1:(NT+i)]],  test=df[SEMANA%in%semanas[(NT+i+1):(NT+i+N)]])
}



i=1
train=data.table::copy(data[[i]]$train)
test=data.table::copy(data[[i]]$test)
...
# Casos cofirmados
data.table::setnames(train,"CASOS_CONFIRMADO_COMUNA","CASOS")
train<-funciones_ordenar_fecha_semana(train,tipo="FECHA")
COVID_CASOS_CONFIRMADOS_COMUNAS_SPLIT_TS<-funciones_create_ts(train,var="CASOS",frecuencia="diaria")
COVID_CASOS_CONFIRMADOS_COMUNAS_SPLIT_TS_MOD<-funciones_modelos_var(COVID_CASOS_CONFIRMADOS_COMUNAS_SPLIT_TS)
COVID_CASOS_CONFIRMADOS_COMUNAS_SPLIT_TS_MOD_PREDICT<-funciones_prediccion_var_mod(COVID_CASOS_CONFIRMADOS_COMUNAS_SPLIT_TS_MOD,n.ahead=nDias,CI=0.95)

data.table::setnames(test,"CASOS_CONFIRMADO_COMUNA","CASOS")
test<-funciones_ordenar_fecha_semana(test,tipo="FECHA")
COVID_CASOS_CONFIRMADOS_COMUNAS_SPLIT_TS_TEST<-funciones_create_ts(test,var="CASOS",frecuencia="diaria")

#casos Nuevos totales
data.table::setnames(train,"CASOS_NUEVOS_TOTALES","CASOS_CONFIRMADO_REGION",skip_absent = TRUE)
COVID_CASOS_CONFIRMADOS_REGION_SPLIT_TS<-funciones_create_ts(train,var="CASOS_CONFIRMADO_REGION",frecuencia="diaria")
COVID_CASOS_CONFIRMADOS_REGION_SPLIT_TS_MOD<-funciones_modelos_var(COVID_CASOS_CONFIRMADOS_REGION_SPLIT_TS)
COVID_CASOS_CONFIRMADOS_REGION_SPLIT_TS_MOD_PREDICT<-funciones_prediccion_var_mod(COVID_CASOS_CONFIRMADOS_REGION_SPLIT_TS_MOD,n.ahead=nDias,CI=0.95)

data.table::setnames(test,"CASOS_NUEVOS_TOTALES","CASOS_CONFIRMADO_REGION",skip_absent = TRUE)
test<-funciones_ordenar_fecha_semana(test,tipo="FECHA")
COVID_CASOS_CONFIRMADOS_REGION_SPLIT_TS_TEST<-funciones_create_ts(test,var="CASOS_CONFIRMADO_REGION",frecuencia="diaria")


#PCR
COVID_CASOS_CASOS_NUEVOS_REGIONAL_SPLIT_TS<-funciones_create_ts(train,var="CASOS_CONFIRMADO_REGION",frecuencia="diaria")
